...
-**Day 12+13: Submission Complete - Lessons from the AI Development Trenches**
Well, that was painful! Successfully submitted Canøpy to the World's Largest Hackathon, though the app decided to completely stop working during the final demo recording - couldn't even save entries to the database. Classic hackathon moment. So I guess developers don't need to worry about their jobs yet! I'm 100% sure a skilled developer would have taken less than 13-20 hours to properly build what I sort of half-arsed here over two weeks.
The most interesting discovery: Bolt.new took my idea and got it 80% there with very little input, but iterating beyond that 80% became a nightmare. Same story with Gamma for slides - gorgeous first output, then editing became incredibly difficult. This seems to be a fundamental characteristic of LLMs: impressive initial quality, but rarely 100% there, and iteration is painful because of natural output variety. It's much more like teaching than designing - you can steer and instruct, find better ways to communicate, but ultimately the result has its own life. There's inherent beauty in that unpredictability, even when it's frustrating.

The psychology-conscious nutrition app concept still feels viable to me, though I'm realistic that it's a simple idea many people could execute. I just hope someone builds it properly so I can actually use it! Despite the frustration, learned a ton and already excited for the next project. Key learnings: test LLM APIs thoroughly before app integration, create proper designs before handing to LLMs, work with an engineer next time, try Claude's API for more control, and appreciate that Bolt gave me skills I didn't have before.

Time to rest and plan the next build. 🌿
